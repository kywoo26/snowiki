...
def test_known_item_lookup_uses_path_bias_session_weight_and_limit_multiplier() -> None:
    ...
    hits = known_item_lookup(index, "config path", limit=2)

    assert [hit.document.id for hit in hits] == ["first", "second"]
...
def test_topical_recall_uses_neutral_kind_weights_and_kind_blending() -> None:
    ...
    hits = topical_recall(index, "retrieval plan", limit=3)
...
def test_temporal_recall_detects_yesterday_window_and_applies_limit_multiplier() -> None:
    ...
    assert [hit.document.id for hit in hits] == ["old"]
    assert len(index.calls) == 1
    request = index.calls[0]
